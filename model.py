...
class Comment(Base, _BaseMixin):
    ...
    text = Column(Unicode(16), unique=False)
    # relations
...
class Profile(Base, _BaseMixin):
    ...
    # relations
    rel_profileevent = relationship("ProfileEvent", collection_class=attribute_mapped_collection('group'),
                                  cascade='all,delete-orphan',
                                  backref='profile',
                                  )
    events = association_proxy('rel_profileevent', 'comment', creator=lambda event, comment: ProfileEvent(event=event, comemnt=comment))

class Event(Base, _BaseMixin):
    __tablename__ = 'events'
    # columns
    id = Column(Integer, primary_key=True, autoincrement=True)
    name = Column(Unicode(16), unique=False)
    # relations
    rel_profileevent = relationship("ProfileEvent", collection_class=attribute_mapped_collection('user'),
                                  cascade='all,delete-orphan',
                                  backref='event',
                                  )
    users = association_proxy('rel_profileevent', 'comment', creator=lambda profile, comment: ProfileEvent(profile=profile, comemnt=comment))
...
class ProfileEvent(Base, _BaseMixin):
    ...
    group_id = Column(Integer, ForeignKey(Event.__tablename__ + '.id', ondelete='CASCADE'), nullable=False)
...
class Interest(Base, _BaseMixin):
	...
	# columns
	id = Column(Integer, primary_key=True, autoincrement=True)
	tag = Column(Unicode(16), unique=True)
